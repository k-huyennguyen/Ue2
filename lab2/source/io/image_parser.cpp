...
BitmapImage ImageParser::read_bitmap(const std::filesystem::path& file_path) {
	...
	bitmap_buffer.resize(bitmap_height * bitmap_width * 3);

	file_reader.clear();
	file_reader.seekg(bfOffBits);
	file_reader.read(bitmap_buffer.data(), bitmap_height * bitmap_width * 3);
...
void ImageParser::write_bitmap(const std::filesystem::path& file_path, const BitmapImage& bitmap) {
	...
	auto bfSize = std::uint32_t{ 3 * bitmap.get_height() * bitmap.get_width() + 54 };
