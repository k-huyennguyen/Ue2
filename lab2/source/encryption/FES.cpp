...
BitmapImage FES::encrypt_parallel_coarse(const BitmapImage& original_image, const FES::key_type& encryption_key, int num_of_threads) {
    const auto height = original_image.get_height();
    const auto width = original_image.get_width();

    auto encrypted_image = BitmapImage{ height, width };
#pragma omp parallel for num_threads(num_of_threads) default(none) shared(height, width, original_image, encryption_key, encrypted_image)
    for (auto y = std::uint32_t(0); y < height; y += 3) {
        for (auto x = std::uint32_t(0); x < width; x += 48) {
            const auto& current_block = get_block(y, x, original_image);
            const auto& previous_block = get_previous_block(y, x, encryption_key, original_image);
...
BitmapImage FES::encrypt_parallel_fine(const BitmapImage &original_image, const FES::key_type &encryption_key, int num_of_threads) {
    const auto height = original_image.get_height();
    const auto width = original_image.get_width();
    ...
    for (auto y = std::uint32_t(0); y < height; y += 3) {
#pragma omp parallel for num_threads(num_of_threads) default(none) shared(width, y, original_image, encryption_key, encrypted_image)
        for (auto x = std::uint32_t(0); x < width; x += 48) {
            const auto& current_block = get_block(y, x, original_image);
            const auto& previous_block = get_previous_block(y, x, encryption_key, original_image);
...
BitmapImage FES::encrypt_parallel(const BitmapImage &original_image, const FES::key_type &encryption_key, int num_of_threads) {
    const auto height = original_image.get_height();
    const auto width = original_image.get_width();

    auto encrypted_image = BitmapImage{ height, width };

#pragma omp parallel for num_threads(num_of_threads) default(none) shared(height, width, original_image, encrypted_image, encryption_key) collapse(2)
    for (auto y = std::uint32_t(0); y < height; y += 3) {
        for (auto x = std::uint32_t(0); x < width; x += 48) {
            const auto& current_block = get_block(y, x, original_image);
            const auto& previous_block = get_previous_block(y, x, encryption_key, original_image);
