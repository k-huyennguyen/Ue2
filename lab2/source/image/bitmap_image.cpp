...
BitmapImage BitmapImage::transpose_parallel(int num_of_threads) const {
    auto transposed_image = BitmapImage(width, height);
#pragma omp parallel for num_threads(num_of_threads) default(none) shared(transposed_image) collapse(2)
    for (auto y = std::uint32_t(0); y < height; y++) {
        for (auto x = std::uint32_t(0); x < width; x++) {
            transposed_image.set_pixel(x, y, get_pixel(y, x));
        }
...
BitmapImage BitmapImage::get_grayscale_parallel(int num_of_threads) const {
    auto gray_image = BitmapImage(height, width);

#pragma omp parallel for num_threads(num_of_threads) default(none) shared(gray_image) collapse(2)
    for (auto y = std::uint32_t(0); y < height; y++) {
        for (auto x = std::uint32_t(0); x < width; x++) {
            const auto pixel = get_pixel(y, x);
            const auto r = pixel.get_red_channel();
